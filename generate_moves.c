...
void gen_king_moves(int src){
    ...
    if(src>=9 && src%8!=0) try_append_move((move){src, src-9,0,0,0,update_castle_flags});
    if(src>=8) try_append_move((move){src, src-8,0,0,0,update_castle_flags});
    if(src>=8 && src%8 != 7) try_append_move((move){src, src-7,0,0,0,update_castle_flags});
    if(src%8 != 0) try_append_move((move){src, src-1,0,0,0,update_castle_flags});
    if(src%8 != 7) try_append_move((move){src, src+1,0,0,0,update_castle_flags});
    if(src<56 && src%8 != 0) try_append_move((move){src, src+7,0,0,0,update_castle_flags});
    if(src < 56) try_append_move((move){src, src+8,0,0,0,update_castle_flags});
    if(src < 56 && src%8 != 7) try_append_move((move){src, src+9,0,0,0,update_castle_flags});
    ...
    if(src==60 && board_md.castle_flags & 8 && !is_in_check(0) && !board[61] && !board[62]){
        ...
        if(no_c_through_check) append_move((move){60,62,0,0,0,12});
    ...
    if(src == 60 && board_md.castle_flags & 4 && !is_in_check(0)&& !board[57] && !board[58] && !board[59]){
        ...
        if(no_c_through_check) append_move((move){60,58,0,0,0,12});
    ...
    if(src==4 && board_md.castle_flags & 2 && !is_in_check(1) && !board[5] && !board[6]){
        ...
        if(no_c_through_check) append_move((move){4,6,0,0,0,3});
    ...
    if(src==4 && board_md.castle_flags & 1 && !is_in_check(1) && !board[1] && !board[2] && !board[3]){
        ...
        if(no_c_through_check) append_move((move){4,2,0,0,0,3});
...
void gen_pawn_moves(int src){
    if(color=='w'){
        ...
        if(src >=48 && src < 56){
            //If room for DPP, DPP
            if(!board[src-8] && !board[src-16]) append_move((move){src, src-16,0,1,src-8,0});
        }

        //En passant
        if(board_md.ep_right<24 && src>16 && (src == board_md.ep_right+7 || src==board_md.ep_right+9)){
            //avoid edge wrap-around
            if(board_md.ep_right/8 == src/8 - 1) append_move((move){src, board_md.ep_right, 0,1,0,0});
        }
        ...
        if(src<16){
            //push
            if(!board[src-8]){
                for(int promo_piece = 2; promo_piece <=5; promo_piece++) append_move((move){src,src-8, promo_piece, 1, 0, 0});
            }

            //capture
            if(src%8 != 7 && board[src-7] && board[src-7] > 16){
                for(int promo_piece = 2; promo_piece <= 5; promo_piece++) append_move((move){src,src-7, promo_piece, 1, 0, 0});
            }
            if(src %8 != 0 && board[src-9] && board[src-9] > 16){
                for(int promo_piece = 2; promo_piece <= 5; promo_piece++) append_move((move){src,src-9, promo_piece, 1, 0, 0});
            }
        //non promotion
        } else {
            //push
            if(!board[src-8]) append_move((move){src, src-8, 0, 1, 0, 0});

            //capture
            if( src%8!=7 && board[src-7] && board[src-7] >16) append_move((move){src,src-7, 0, 1, 0, 0});
            if( src%8 != 0 && board[src-9] && board[src-9] > 16) append_move((move){src,src-9, 0, 1, 0, 0});
        }
    } else {
        //correct position for DPP
        if(src >=8 && src < 16){
            ...
            if(!board[src+8] && !board[src+16]){
                append_move((move){src, src+16,0,1,src+8,0});
            }
        }
        ...
        //non-promotion
            ...
            //capture
            if(src%8 != 0 && board[src+7] && board[src+7]<16) append_move((move){src,src+7, 0, 1, 0, 0});
            if(src%8 != 7 && board[src+9] && board[src+9]<16) append_move((move){src,src+9, 0, 1, 0, 0});
        }
    }
}
...
void gen_bishop_moves(int src){
    if(src%8!=0){
        for(int i = src-9; ; i-=9){
            if(i<0) break;
            if(board[i]){
                if(board[i] > 16 && color == 'w') append_move((move){src, i, 0, 1, 0, 0});
                else if(board[i] < 16 && color == 'b') append_move((move){src, i, 0, 1, 0, 0});
                break;
            }
            append_move((move){src,i,0,0,0,0});
            if(i%8==0) break;
        }
    }
    if(src%8!=7){
...
void gen_knight_moves(int src){
    if(src>=16 && src%8>0) try_append_move((move){src, src-17,0,0,0,0});
    if(src>=16 && src%8<7) try_append_move((move){src, src-15,0,0,0,0});
    if(src>=8 && src%8>1) try_append_move((move){src, src-10,0,0,0,0});
    if(src>=8 && src%8<6) try_append_move((move){src, src-6,0,0,0,0});
    if(src<56 && src%8>1) try_append_move((move){src, src+6,0,0,0,0});
    if(src<56 && src%8<6) try_append_move((move){src, src+10,0,0,0,0});
    if(src<48 && src%8>0) try_append_move((move){src, src+15,0,0,0,0});
    if(src<48 && src%8<7) try_append_move((move){src, src+17,0,0,0,0});
}

void gen_rook_moves(int src){
    for(int i = src-8; i>-1; i-=8){
        if(board[i]){
            try_append_move((move){src, i, 0, 0, 0, 0});
            break;
        }
        append_move((move){src, i, 0, 0, 0, 0});
    }
